...
def make_pdf(ticker, content_dict):
    pdf = FPDF()
    ...
    for panel_name, text in content_dict.items():
        if text:
            # Section Titles
            pdf.set_font("Helvetica", "B", 12)
            pdf.cell(0, 10, panel_name.encode('latin-1', 'ignore').decode('latin-1'), ln=True)
            pdf.ln(2)
            
            # Body Paragraph Text
            pdf.set_font("Helvetica", "", 10)
            
            # Clean out markdown formatting and high noise symbols for basic PDF canvas placement
...
if check_password():
    ...
    def generate_analysis_layer(ticker, prompt_text):
        ...
        models_to_try = ["gemini-3.5-flash", "gemini-2.5-flash", "gemini-3.1-flash-lite"]
        max_retries = 10
    ...
    # Initialize permanent data dictionary across application reruns
    ...
    with st.form(key="research_panel_form"):
        ...
        st.markdown("Enter both the company name and the ticker symbol.")
    ...
    if submit_button and ticker:
        ...
        with st.spinner(f"🔢 Running valuation math specifically targeted to Phase {phase_num}..."):
            ...
            Step 2: Source recent filings (10-Q/10-K or international IR Annual Reports/investor presentations if non-US). Gather current share price, diluted share count, cash, debt, and consensus NTM metrics. Calculate EV = Market Cap + Debt - Cash.
            Step 3: Execute valuation models explicitly matching the Phase {phase_num} methodologies and industry overrides.
        ...
        if calculated_status is None:
            if phase_num in ["2", "3"]:
                if (has_moat_valid and moat_widening and high_growth_potential and 
                    ...
                    rule_justification = f"Phase {phase_num} Early Growth Play matching all structural moat expansion parameters."
                else:
                    calculated_status = "⏳ ADD TO WATCHLIST"
                    rule_justification = f"Phase {phase_num} Growth asset, but missing premium acceleration metrics."
            elif phase_num == "4":
                if (has_moat_valid and moat_widening and high_growth_potential and 
                    growth_accelerating and metrics_pass and risk_acceptable and financials_acceptable):
                    if valuation_fair_or_under:
                        calculated_status = "🚀 DEEP DIVE ASAP"
                        rule_justification = "Phase 4 Mature asset meeting premium criteria with a clear valuation margin of safety."
                    else:
                        calculated_status = "⏳ ADD TO WATCHLIST"
                        rule_justification = "Cleared quality bars, but flagged as Overvalued for a Phase 4 profile."
                else:
                    calculated_status = "❌ PASS (Not Good Enough)"
                    rule_justification = "Phase 4 mature profile failing to meet core structural framework requirements."
            else:
                calculated_status = "⏳ ADD TO WATCHLIST"
                rule_justification = "System fallback logic triggered."
        ...
        with st.expander("⚖️ Panel #8: Final Investment Decision", expanded=True):
            ...
            p8_prompt = f"""
            CRITICAL OPERATIONAL INSTRUCTION: You are the Chief Investment Officer. Write the executive synthesis for target asset: '{ticker}' (Phase Context: Phase {phase_num}).
